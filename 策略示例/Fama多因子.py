...
def on_data(context):
    if dt.datetime.strftime(context.now, '%Y-%m-%d') not in context.month_begin:  # 调仓频率为月
        return
    factor = get_reg_factor(context.reg_factor[0], target_indices=[x for x in range(300)], length=1, df=True)
    ...
    kdata = get_reg_kdata(context.reg_kdata[0], target_indices=[x for x in range(300)], length=context.date + 1,
    ...
    hml = hml_b - hml_s
    market_close = get_reg_kdata(context.reg_kdata[0], target_indices=[300],
...
if __name__ == '__main__':
    ...
    targetlist.append('sse.000300')
    run_backtest(strategy_name='Fama多因子',
                 file_path='Fama多因子.py',
