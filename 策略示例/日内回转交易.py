...
def on_data(context):
    ...
    if context.total >= 0:
        ...
        if recent_data.isna().any():
            return
        # 计算MACD线
        # ma_short = talib.MA(recent_data, timeperiod=15)
        # ma_long = talib.MA(recent_data, timeperiod=30)
        # sma_now = ma_short.iloc[-1]
        # sma_1 = ma_short.iloc[-2]
        # lma_now = ma_long.iloc[-1]
        # lma_1 = ma_long.iloc[-2]
        macd = talib.MACD(recent_data.astype(float))[2].iloc[-1]
        # 根据MACD>0则开仓,小于0则平仓
        # if (sma_now > lma_now) and (sma_1 < lma_1):
        if macd > 0:
        #if (macd > 0) and (macd_1 < 0):
        ...
        # elif (sma_now < lma_now) and (sma_1 > lma_1):
        elif macd < 0:
        #elif (macd < 0) and (macd_1 > 0):
